...
from PyQt5.QtGui import QPixmap, QPainter, QColor, QPen,QMouseEvent
...
class MapBuilderWindow(QMainWindow):
    def __init__(self):
        ...
        self.map_label.setAlignment(Qt.AlignLeft)###mark
        self.scroll_area.setWidget(self.map_label)
        main_layout.addWidget(self.scroll_area)

        self.map_label.mousePressEvent=self.label_mousePressEvent###mark
        self.map_label.mouseMoveEvent=self.label_mouseMoveEvent###mark
        # 畫筆與橡皮擦工具
    ...
    def show_map_image(self, directory, map_file):
        ...
        # 初始化編輯用地圖圖片
        self.edited_image_path = map_path.replace('.pgm', '_edited.png')  # 使用 PNG 格式保存編輯檔案
        if not os.path.exists(self.edited_image_path):  # 如果沒有已有編輯檔案，複製原始地圖
            pixmap = QPixmap(map_path)
            pixmap.save(self.edited_image_path)
    ...
    #         original_pixmap = QPixmap(self.edited_image_path)  # 載入保存的編輯地圖
    ...
    #         )
    #         self.map_label.setPixmap(scaled_pixmap)  # 更新顯示
    #         self.update_scale_label()
    def update_map_transform(self):
        if self.edited_image_path:
            # 載入保存的編輯地圖
            original_pixmap = QPixmap(self.edited_image_path)
            scaled_pixmap = original_pixmap.scaled(
                int(original_pixmap.width() * self.current_scale),
                int(original_pixmap.height() * self.current_scale),
                Qt.KeepAspectRatio,
    ...
    def draw_grid(self):
        ...
        pen = QPen(QColor(200, 200, 200), 1, Qt.SolidLine)  # 固定淺灰色實線
        painter.setPen(pen)

        grid_spacing = 20  # 固定格線間距（以像素為單位）

        # 繪製垂直和水平格線
        for x in range(0, self.map_label.width(), grid_spacing):
            painter.drawLine(x, 0, x, self.map_label.height())  # 垂直線
        for y in range(0, self.map_label.height(), grid_spacing):
            painter.drawLine(0, y, self.map_label.width(), y)  # 水平線
    ...
    #def mouseMoveEvent(self, event):
            ...
            #self.modify_map(event)
    def label_mousePressEvent(self,event: QMouseEvent):
        #if event.button() == Qt.LeftButton:  # 检查是否是左键点击
        self.map_label_x = event.x()
        self.map_label_y = event.y()
        #print('label_xy=',self.map_label_x,self.map_label_y)
        self.modify_map(event)
    def label_mouseMoveEvent(self,event: QMouseEvent):
        self.map_label_x = event.x()
        self.map_label_y = event.y()
        #print('label_xy=',self.map_label_x,self.map_label_y)
        self.modify_map(event)
    ...
    def modify_map(self, event):
        ...
        # 計算點擊位置在原始圖片中的座標
        x = int(event.pos().x() / self.current_scale)
        y = int(event.pos().y() / self.current_scale)

        # 在保存的圖片中進行修改
        image = QPixmap(self.edited_image_path).toImage()
        ...
        # 保存修改到文件
        image.save(self.edited_image_path)
    ...
    def update_scale_label(self):
        if self.resolution:
            scale_length = (20 * self.resolution) / self.current_scale
            self.scale_label.setText(f"比例尺: {round(scale_length, 2):.2f} m (20 像素)")
        else:
