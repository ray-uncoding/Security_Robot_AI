...
class MapBuilderWindow(QMainWindow):
    ...
    def save_map(self):
        # 彈出輸入框讓用戶輸入地圖名稱
        map_name, ok = QInputDialog.getText(self, "保存地圖", "輸入地圖名稱（例如：1F）")
        if not ok or not map_name.strip():
            QMessageBox.warning(self, "保存取消", "地圖名稱無效，保存操作已取消。")
            return

        # 確定保存路徑
        save_path = f"/home/sr/wheeltec_ros2/src/wheeltec_robot_nav2/map/{map_name.strip()}"
        ...
        try:
            ...
            QMessageBox.critical(self, "保存失敗", f"發生錯誤：{e}")
    ...
    #     map_directory = "C:/Users/User/Documents/UI"#"/home/sr/wheeltec_ros2/src/wheeltec_robot_nav2/map"####
    #     try:
    ...
    def modify_map(self, event):
        ...
        x = self.map_label_x
        y = self.map_label_y
        print('event.pos().x()=',event.pos().x())
        print('self.map_label.x()=',self.map_label.x())#都是0
        print('event.pos().y()=',event.pos().y())
        print('self.map_label.y()=',self.map_label.y())#都是0
        #self.map_label.mousePressEvent=self.label_mousePressEvent
        #print('self.map_label.mousePressEvent.x()=',self.map_label.mousePressEvent(event).x() )
    ...
    def save_edited_map(self):
        if not self.edited_pixmap:
            QMessageBox.critical(self, "保存失敗", "目前沒有可保存的地圖，請先載入地圖或進行修改。")
            return
        ...
        # 確定保存路徑
        save_path = f"/home/sr/wheeltec_ros2/src/wheeltec_robot_nav2/map/{map_name.strip()}.png"

        try:
            if not self.edited_pixmap.save(save_path):  # 保存圖片到指定路徑
                QMessageBox.critical(self, "保存失敗", "地圖保存失敗，請檢查文件格式或目標路徑。")
            else:
                QMessageBox.information(self, "保存成功", f"地圖已保存至: {save_path}")
        except Exception as e:
            QMessageBox.critical(self, "保存失敗", f"發生錯誤：{e}")
