...
class OBSStreamCapture:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            display_scale: 顯示縮放比例 (1.0 = 原始大小)
            max_display_width: 最大顯示寬度
            max_display_height: 最大顯示高度
        """
        ...
        self.original_frame = None  # 保存原始高解析度畫面
        self.display_frame = None   # 保存縮放後的顯示畫面
            ...
            while self.is_running:
                try:
                    ...
                    if ret and frame is not None:
                        ...
                        # 使用縮放後的畫面作為當前幀（用於顯示）
                        self.current_frame = self.display_frame if self.display_frame is not None else frame
    ...
    def _update_display_frame(self, frame: np.ndarray):
        ...
        try:
            ...
            # 第一次設置或尺寸改變時計算縮放參數
            if (self.original_width != w or self.original_height != h or 
                self.display_width is None or self.display_height is None):
                ...
                if self.display_scale != 1.0:
                    # 使用指定的縮放比例
                    self.display_width = int(w * self.display_scale)
                    self.display_height = int(h * self.display_scale)
                else:
                    # 使用最大尺寸限制
                ...
                logger.info(f"顯示尺寸: {self.original_width}x{self.original_height} -> {self.display_width}x{self.display_height}")
            
            # 如果需要縮放，進行快速縮放
            if (self.display_width != self.original_width or 
            ...
            logger.error(f"更新顯示畫面失敗: {e}")
            self.display_frame = frame  # 回退到原始畫面
