...
class MapWindow(QMainWindow):
    ...
    def stop_process(self):
        if self.terminal_process:
            # 終止當前導航進程
            print(f"停止導航進程，PID: {self.terminal_process.pid}")
        ...
        # 啟動新進程執行 SLAM map 指令
        try:
            print("啟動 SLAM map 程序...")
            command = "cd ~/wheeltec_ros2 && source install/setup.bash && ros2 launch wheeltec_slam_toolbox online_async_launch.py"
            subprocess.Popen(f"bash -c '{command}'", shell=True, executable='/bin/bash')
            print("SLAM map 程序已啟動")
        except Exception as e:
            print(f"啟動 SLAM map 程序失敗: {e}")
