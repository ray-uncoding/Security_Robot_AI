...
class OCRCameraWidget(QWidget):
    ...
    def _init_camera_system(self):
        ...
        try:
            ...
            # 使用較小的解析度以提升性能，減少卡頓
            self.receiver = FrameReceiver(stream_url=camera_rtmp_url, width=960, height=1920)  # 縮小一半
    ...
    def _cleanup_old_frames(self):
        """清理舊幀以減少延遲"""
        if self.receiver:
            # 激進的幀清理：快速獲取多次最新幀，徹底丟棄積壓
            latest_frame = None
            for _ in range(10):  # 最多嘗試10次獲取最新幀
    ...
    def update_frame(self):
        ...
        # 4.5.2 從 FrameReceiver 獲取最新幀，並清理積壓
        frame = None
        # 嘗試獲取最新的幀，跳過積壓的舊幀
        for _ in range(5):
            latest = self.receiver.get_latest_frame()
        ...
        # 4.5.2.1 如果影像太大，先縮小以提升性能
        h, w = frame.shape[:2]
        if w > 1920 or h > 1920:  # 如果任一邊超過1920像素
            scale_factor = min(1920/w, 1920/h)
        ...
        # 4.5.7 更新 CUDA GPU 狀態（每200幀一次，降低頻率）
